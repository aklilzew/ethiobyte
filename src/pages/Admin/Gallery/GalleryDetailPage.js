...
const GalleryDetailPage = () => {
  ...
  const getMediaUrl = (path) => {
    ...
    return path ? `${baseUrl}/${path.replace(/\\/g, '/')}` : null;
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadSuccess(`${response.data.count} media item(s) added successfully!`);
      setSelectedFiles([]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 w-full h-full z-0">
        {item.media_type === 'image' && mainMediaUrl ? (
          <img
            ...
            loading="lazy"
            onError={(e) => {
              e.target.src = 'https://via.placeholder.com/300/E2E8F0/9CA3AF?text=Img Error';
          ...
          <div className="w-full h-full flex flex-col items-center justify-center bg-gray-200 text-gray-600 group-hover/card:bg-gray-300 p-2 transition-colors">
            <FiFileText size={40} />
            <span className="text-xs mt-2 text-center line-clamp-2">{mediaItemCaption}</span>
          </div>
        ) : (
      ...
      {/* Play Icon Overlay (for videos only) */}
      {item.media_type === 'video' && (
        <div className="absolute inset-0 flex items-center justify-center bg-black/40 transition-opacity duration-300 z-10 opacity-0 group-hover/card:opacity-100">
          <FiPlayCircle size={48} className="text-white" />
        </div>
      )}

      {/* Caption and Delete Button Overlay */}
